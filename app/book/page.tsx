    ...
    return (
      <div className="min-h-screen bg-deep-black flex items-center justify-center px-4">
        ...
        >
          ...
          <button onClick={() => router.push('/')} className="text-gold uppercase tracking-widest border-b border-gold pb-1 hover:text-white hover:border-white transition-colors">
            Return Home
          </button>
        </motion.div>
      </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            {step === 2 && (
              <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                ...
                {/* File Upload */}
                <div className="mb-8 border border-dashed border-gray-700 p-6 text-center rounded hover:border-gold transition-colors cursor-pointer bg-black/50 relative">
                    <UploadIcon className="w-8 h-8 text-gray-500 mx-auto mb-2" />
                    <p className="text-gray-400 text-sm mb-2">Upload a photo of your item (Optional)</p>
                    {photoFile && <p className="text-gold text-xs">{photoFile.name}</p>}
                    <input 
                      type="file" 
                      accept="image/*"
