    ...
    <div className="pt-28 pb-20 min-h-screen bg-deep-charcoal px-4 md:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-10">
          <div>
            <h1 className="text-4xl font-serif text-white mb-2">Blog Management</h1>
          ...
          <button
            ...
            className="flex items-center space-x-2 bg-gold text-black px-6 py-3 hover:bg-white transition-colors uppercase tracking-widest text-sm font-bold"
          >
        ...
        <div className="bg-black border border-gray-800 rounded-lg overflow-hidden">
          {posts.length === 0 ? (
            ...
            <div className="divide-y divide-gray-800">
              {posts.map((post) => (
                <div key={post.id} className="p-6 hover:bg-gray-900/30 transition-colors">
                  <div className="flex justify-between items-start">
                    <div className="flex-1">
                      ...
                      <p className="text-gray-400 text-sm mb-2">{post.excerpt || 'No excerpt'}</p>
                    ...
                    <div className="flex space-x-2 ml-4">
                      <button
                        ...
                        className="px-4 py-2 border border-gray-800 text-gray-400 hover:text-white hover:border-gray-700 transition-colors text-xs uppercase tracking-widest"
                      >
                        {post.published ? 'Unpublish' : 'Publish'}
                      </button>
                      <button
                        onClick={() => handleEdit(post)}
                        className="px-4 py-2 border border-gold text-gold hover:bg-gold hover:text-black transition-colors text-xs uppercase tracking-widest"
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => handleDelete(post.id)}
                        className="px-4 py-2 border border-red-900 text-red-400 hover:bg-red-900 hover:text-white transition-colors text-xs uppercase tracking-widest"
                      >
                        Delete
                      </button>
                    </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4">
          <div className="bg-deep-charcoal border border-gold/30 max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-deep-charcoal border-b border-gray-800 p-6 flex justify-between items-center">
              <h2 className="text-3xl font-serif text-white">{editingPost ? 'Edit Post' : 'New Post'}</h2>
              <button
                onClick={() => {
                  setIsModalOpen(false);
                  setEditingPost(null);
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div>
                ...
                <textarea
                  ...
                  required
                  rows={15}
                  className="w-full bg-black border border-gray-800 text-white px-4 py-3 focus:outline-none focus:border-gold transition-colors resize-none font-mono text-sm"
                />
              ...
              <div className="flex justify-end space-x-4 pt-4 border-t border-gray-800">
                ...
                <button
                  type="submit"
                  className="px-6 py-3 bg-gold hover:bg-white hover:text-black text-black transition-colors uppercase tracking-widest text-sm font-bold"
                >
